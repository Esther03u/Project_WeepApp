<section>
    <header class="mb-4">
        <h4 class="text-dark fw-bold">เปลี่ยนรหัสผ่าน</h4>
        <p class="text-muted small">ตรวจสอบให้แน่ใจว่าบัญชีของคุณใช้รหัสผ่านแบบสุ่มที่ยาวและปลอดภัย</p>
    </header>
    ...
    <form method="post" action="{{ route('password.update') }}">
        ...
        <div class="mb-3">
            <label for="update_password_current_password" class="form-label fw-semibold">รหัสผ่านปัจจุบัน</label>
        ...
        <div class="mb-3">
            <label for="update_password_password" class="form-label fw-semibold">รหัสผ่านใหม่</label>
        ...
        <div class="mb-4">
            <label for="update_password_password_confirmation" class="form-label fw-semibold">ยืนยันรหัสผ่านใหม่</label>
        ...
        <div class="d-flex align-items-center gap-3">
            <button type="submit" class="btn btn-dark px-4">อัปเดตรหัสผ่าน</button>
            ...
            @if (session('status') === 'password-updated')
                <span class="text-success text-sm fw-medium" x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)">
                    <i class="bi bi-check-circle me-1"></i>อัปเดตสำเร็จ
                </span>
            @endif
        </div>
    </form>
